...
#define HTTPD_VERSION_STRING	     "daimapay/1.0"
...
namespace httpclient
{
	...
	ethpool::simple_http s(executor);
	ethpool::http_request req{ boost::beast::http::verb::get, "/", 11 };
	req.set(boost::beast::http::field::host, "www.baidu.com");
	req.set(boost::beast::http::field::user_agent, HTTPD_VERSION_STRING);
	s.async_perform("https://www.baidu.com", req, [](boost::system::error_code ec, ethpool::http_response res) {
		if (!ec)
		{
	...
	*/
	...
	public:
		simple_http(net::executor executor, bool check_cert = true)
			: m_executor(executor)
	...
	public:
		net::executor& get_executor(){ return m_executor;}
		...
		// 下载百分比.
		boost::optional<double> percent()
		{
			return m_download_percent;
		}

		// 下载剩余大小, 如果服务器提供了内容长度.
		boost::optional<std::size_t> content_length_remaining()
		{
		...
		{
			initiate_do_perform(simple_http* p)
				: m_simple_http(p)
			{}
			...
			void operator()(Handler&& handler, const std::string& url, const std::string& socks, http_request& req) const
			{
				...
				auto executor = boost::asio::get_associated_executor(handler, shttp->m_executor);
				boost::asio::detail::non_const_lvalue<Handler> handler2(handler);

				net::spawn(shttp->m_executor, [shttp, url, socks, &req, executor, handler = std::move(handler2.value)]
				(net::yield_context yield) mutable
				{
					http_response result;
					auto ec = shttp->do_async_perform(yield, req, result, url, socks);
	...
	private:
		...
		template<class S>
		boost::system::error_code async_read_http(S& stream, beast::flat_buffer& buffer, http_response& msg, net::yield_context yield)
		{
	...
	private:
		...
		boost::optional<double> m_download_percent;
		boost::optional<std::size_t> m_content_lentgh;
		boost::optional<std::size_t> m_content_lentgh_remaining;
